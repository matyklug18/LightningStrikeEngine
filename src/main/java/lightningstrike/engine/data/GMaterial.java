...
import lightningstrike.engine.util.StringLoader;
...
public class GMaterial {
    ...
    private void initShader(Vector4f color) {
        ...
        FF = FF.replace("%maxpointlights%", Integer.toString(DefaultRenderer.MAX_POINT_LIGHTS))
                .replace("%maxdirlights%", Integer.toString(DefaultRenderer.MAX_DIR_LIGHTS))
                .replace("%maxspotlights%", Integer.toString(DefaultRenderer.MAX_SPOT_LIGHTS));
